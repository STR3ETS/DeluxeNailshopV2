...
@php
    /*
    |--------------------------------------------------------------------------
    | Klantenportaal - inloggen (alleen styling)
    |--------------------------------------------------------------------------
    | Nog geen echte authenticatie: het formulier toont bij versturen een
    ...
    <div class="mx-auto max-w-[980px]">
        <div class="load-reveal grid overflow-hidden rounded-[calc(var(--radius)+10px)] shadow-card lg:grid-cols-[.92fr_1.08fr]">
            ...
            <div class="bg-offwhite p-9 sm:p-12" x-data="{ show: false }">
                <h1 class="font-serif text-[clamp(1.7rem,3vw,2.2rem)] leading-[1.15] font-normal">Welkom <em class="text-primary italic">terug</em></h1>
                <p class="mt-2.5 text-[.92rem] leading-[1.65] font-light text-dark-soft">Log in met je e-mailadres en wachtwoord om verder te gaan.</p>

                <form method="POST" action="{{ route('login.attempt') }}" class="mt-8 flex flex-col gap-5">
                    @csrf
                    ...
                    <label class="block">
                        ...
                        <div class="relative">
                            ...
                            <input :type="show ? 'text' : 'password'" name="password" required placeholder="••••••••" autocomplete="current-password"
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <label class="flex cursor-pointer items-center gap-2.5 text-[.85rem] text-dark-soft">
                            <input type="checkbox" name="remember" class="h-4 w-4 accent-primary"> Onthoud mij
                        </label>
                        <a href="{{ url('/wachtwoord-vergeten') }}" class="text-[.85rem] font-medium text-primary-deep transition-colors hover:text-primary">Wachtwoord vergeten?</a>
                    </div>
                    ...
                    <button type="submit" class="mt-1 flex w-full items-center justify-center gap-2.5 rounded-full bg-primary px-7 py-4 text-[.95rem] font-semibold tracking-[.02em] text-white shadow-[0_14px_30px_-12px_color-mix(in_srgb,var(--color-primary)_70%,transparent)] transition-colors hover:bg-primary-deep">
                        Inloggen <i class="fa-light fa-arrow-right"></i>
                    </button>
                </form>
                ...
                <p class="mt-6 text-center text-[.9rem] font-light text-dark-soft">
                    Nog geen account? <a href="{{ url('/registreren') }}" class="font-semibold text-primary-deep transition-colors hover:text-primary">Maak er gratis een aan</a>
                </p>
            </div>
        </div>
    </div>
